...
public class TeamListDataSource {

	private SQLiteDatabase database;
	private TeamSQLiteHelper dbHelper;
	private String[] columns = {};
	private TeamItemDataSource dsTeams;
	
	public TeamListDataSource(Context context, TeamItemDataSource datDsTeams){
	...
	public TeamList updateTeamList(TeamList newList){
		ContentValues values = new ContentValues();
		values.put(TeamSQLiteHelper.COLUMN_LIST_NUMBERS, newList.getNumbersAsString());
		Log.d("TLds", TeamSQLiteHelper.COLUMN_LIST_NAME + "=" + newList.getName());
		//database.replace(TeamSQLiteHelper.TABLE_TEAM_LISTS, TeamSQLiteHelper.COLUMN_LIST_NAME+"="+newList.getName(), values);
		database.update(TeamSQLiteHelper.TABLE_TEAM_LISTS, values, TeamSQLiteHelper.COLUMN_LIST_NAME + "='" + newList.getName() + "'", null);
	...
	public ArrayList<TeamList> getAllTeamLists(){
		...
		while(!cursor.isAfterLast()){
			datList.add(cursorToTeamList(cursor));			cursor.moveToNext();
		}
